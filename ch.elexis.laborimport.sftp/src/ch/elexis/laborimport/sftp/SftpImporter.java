...
public class SftpImporter extends ImporterPage {
	...
	private Result<?> importDirect(){
		...
		String downloadDirPath =
			CoreHub.localCfg.get(Preferences.DL_DIR, CoreHub.getTempDir().toString());
		...
		if (pwd != null && user != null && host != null) {
			try {
				...
				if (vv != null) {
					for (LsEntry entry : vv) {
						...
						if (!fname.matches("[a-zA-Z0-9_\\-\\.\\/]+")) {
							result.add(new Result(SEVERITY.WARNING, 2, "Dateiname " + fname
								+ " enthält ungültige Zeichen", null, true));
							err++;
							continue;
						}
						...
						try {
							...
							err++;
							r =
								new Result<String>(SEVERITY.WARNING, 1, "Fehler beim Import von "
									+ entry.getFilename(), localFile.getAbsolutePath(), true);
						}
				...
				c.quit();
				session.disconnect();
				SWTHelper.showInfo("Import beendet", "Es wurden " + count
				...
				// ExHandler.handle(e);
				SWTHelper.showError("Fehler bei Badena Import", "Es ist ein Fehler aufgetreten",
					e.getMessage());
			}
		}
		return result;
