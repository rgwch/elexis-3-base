...
public class ArtikelstammImporterTest {
	...
	private static final String[] slEntries = new String[] {
		"7680273040281", "7680651600014"
	};
	...
	private void runImport(String baseName){
		...
		if (onlyInFirst != null)
			log.debug("onlyInFirst {} {} isBlackBoxed {} isDeleted {}  ", onlyInFirst.getDSCR(),
				onlyInFirst.getPHAR(), onlyInFirst.isBlackBoxed(), onlyInFirst.isDeleted());
		// Next Check fails why?
		// TODO: assertTrue(onlyInFirst == null || onlyInFirst.isDeleted() );
		ArtikelstammItem withPkgOverride =
			ArtikelstammItem.findByEANorGTIN(gtinWithPkgSizeOverride);
	...
	private ArtikelstammItem setPkgOverride(String gtin){
		...
		log.debug(
			...
			overridePkgSize.get(ArtikelstammItem.FLD_PKG_SIZE));
	...
	private static void checkResettingPrice(String gtin, double expectedPrice){
		...
		log.debug("checkResettingPrice: phar {} gtin {} user {} old {} before restoring {}",
			item.getPHAR(), item.getGTIN(), expectedPrice, item.getPublicPrice());
		item.setUserDefinedPrice(false);
		log.debug("checkResettingPrice: phar {} gtin {} old {} restored {}", item.getPHAR(),
			item.getGTIN(), expectedPrice, item.getPublicPrice());
	...
	private static void checkResettingVerpackungsEinheit(String gtin, int expectedPkgSize){
		...
		log.debug("checkResettingVerpackungsEinheit: phar {} gtin {} old {} before restoring {}",
			item.getPHAR(), item.getGTIN(), expectedPkgSize, item.getVerpackungsEinheit());
	...
	public void setup() throws Exception{
		log.debug("testImportFirst initFromScratch {}",
			AbstractPersistentObjectTest.initFromScratch());
